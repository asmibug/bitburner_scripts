...
export async function main(ns) {
    ...
    if (idx !== undefined && total !== undefined) {
        operation_name += ns.sprintf(" %f/%d", idx, total);
    }
    ...
    if (start - wakeup_time > wake_delay) {
        ...
        ns.tprintf("ERROR %s %s: %s start is late for %d",
                   t, host, operation_name, late_for);
    }
    const res = await ns.weaken(host);
    ...
    if (expected !== undefined && res != 0 && Math.abs(res - expected) > 1e-6) {
        const t = new Date().toLocaleTimeString();
