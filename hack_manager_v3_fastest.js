...
export async function main(ns) {
    ...
    setTMargin(100);  // TODO: remove this
    let tmargin = 100;
    
    function checkSecurityIsMinimal(msg, operation) {
        ...
        if (s0 - s > 0.001) {
            ...
            throw ns.sprintf("%s: security is not minimal: %s\n%f=s != s0=%f",
                             host, msg, s, s0) + info;
        }
    }
    ...
    function runOperation(operation) {
        checkSecurityIsMinimal("before operation start", operation);
        const pid = ns.exec(getFileName(operation), host_for_scripts, operation.threads,
                            host, operation.start);
        const start_time = Date.now();
        checkSecurityIsMinimal("after operation start", operation);
        if (!pid) {
        ...
        while (starts.length && !is_weaken_run) {
            ...
            while (starts.length && starts[0].start <= Date.now()) {
                ...
                if (getOperationDuration(operation.op) != operation.duration) {
                    checkSecurityIsMinimal("when duration has changed", operation);
    ...
    checkSecurityIsMinimal("before starting the main cycle");
